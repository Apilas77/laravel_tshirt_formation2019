...
@else
    <tr class="table-info">
        ...
        <td>
            <a href="{{route('backend_cat_edit',['id'=>$categorie->id])}}"
               class="btn btn-sm btn-primary">Voir / Modifier</a>

            <a href="{{route('backend_cat_delete',['id'=>$categorie->id])}}"
               class="btn btn-sm btn-outline-danger">Supprimer</a>
        </td>
    </tr>
    @if(count($categorie->enfants)>0)
        @each('backend.categories.list_categories', $categorie->enfants, 'categorie')
    @endif
